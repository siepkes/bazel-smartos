...
def _impl(ctx):
    ...
    if (is_illumos):
        # Paths obtained with '/opt/local/gcc13/bin/g++ -E -x c++ - -v < /dev/null'.
        cxx_builtin_include_directories = [
            "/opt/local/gcc13/include/c++/13.2.0",
            "/opt/local/gcc13/include/c++/13.3.0/x86_64-sun-solaris2.11",
            "/opt/local/gcc13/include/c++/13.3.0/backward",
            "/opt/local/gcc13/lib/gcc/x86_64-sun-solaris2.11/13.3.0/include",
            "/opt/local/include",
            "/opt/local/gcc13/include",
            "/opt/local/gcc13/lib/gcc/x86_64-sun-solaris2.11/13.3.0/include-fixed",
            "/usr/include"
        ]
    else:
        cxx_builtin_include_directories = []
    ...
    if is_illumos:
        tool_paths = [
            ...
            tool_path(name = "cpp", path = "/opt/local/gcc13/bin/cpp"),
            # Does not exist on Solaris.
            tool_path(name = "dwp", path = "/usr/bin/dwp"),
            tool_path(name = "gcc", path = "/opt/local/gcc13/bin/gcc"),
            tool_path(name = "gcov", path = "/opt/local/gcc13/bin/gcov"),
