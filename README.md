...
Create a SmartOS container (joyent brand if your on Triton). To build and use Bazel you need an image which has GCC 9. This guide assumes you use image `1d05e788-5409-11eb-b12f-037bd7fee4ee` (20.4.0) with GCC 9. Beware that currently the `9.3.0` path is hardcoded in Bazel path (see known issues).
...
Download [bazel-5.1.1-dist.zip](https://github.com/bazelbuild/bazel/releases/download/5.1.1/bazel-5.1.1-dist.zip) (the Bazel distribution) from the [Bazel releases](https://github.com/bazelbuild/bazel/releases) page.

Next we will prepare our source tree by putting everything together:

```
# mkdir bazel-5.1.1
# unzip bazel-5.1.1-dist.zip -d bazel-5.1.1
# git clone https://github.com/siepkes/bazel-smartos.git
# cd bazel-smartos
# git checkout smartos-5.1.1
# git format-patch -1
# cd ../bazel-5.1.1
# patch -p1 < ../bazel-smartos/0001-Modifications-to-port-Bazel-5.1.1-to-Illumos-SmartOS.patch
```
